...
import { VoiceSettings, AudioConfig, VoiceResponse, ConversationTurn } from './types/voice.types';
...
export class GeminiLiveService {
  ...
  async connect(tenant: string, tenantSettings: any): Promise<void> {
    try {
  ...
  private generateTenantSpecificInstruction(tenant: string, settings: any): string {
...
HUSK: Du arbejder kun med data fra ${tenant} og kan ikke se data fra andre tenants.`;
  ...
  async processVoiceCommand(audioInput: Buffer): Promise<VoiceResponse> {
    if (!this.isConnected) {
