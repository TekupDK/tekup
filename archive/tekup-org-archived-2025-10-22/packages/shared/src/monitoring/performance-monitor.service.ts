...
export class PerformanceMonitorService {
  ...
  private eventBus: any; // Will be injected
  ...
  calculateSystemHealth(): SystemHealth {
    ...
    if (avgResponseTime > 1000) responseTimeScore = 0;
    else if (avgResponseTime > 500) responseTimeScore = 25;
    else if (avgResponseTime > 200) responseTimeScore = 50;
    else if (avgResponseTime > 100) responseTimeScore = 75;

    // Error rate score (lower is better)
    let errorRateScore = 100;
    if (totalErrors > 10) errorRateScore = 0;
    else if (totalErrors > 5) errorRateScore = 25;
    else if (totalErrors > 2) errorRateScore = 50;
    else if (totalErrors > 0) errorRateScore = 75;

    // Throughput score (higher is better)
    let throughputScore = 100;
    if (avgThroughput < 1) throughputScore = 0;
    else if (avgThroughput < 5) throughputScore = 25;
    else if (avgThroughput < 10) throughputScore = 50;
    else if (avgThroughput < 20) throughputScore = 75;
  ...
  private checkThresholds(metric: PerformanceMetric): void {
    const thresholds = this.getThresholds().filter(t => 
      t.metricName === metric.name && t.enabled
    );
   ...
   * Publish metric event
   */
  private publishMetricEvent(metric: PerformanceMetric): void {
    if (this.eventBus) {
   ...
   * Publish alert event
   */
  private publishAlertEvent(alert: PerformanceAlert): void {
    if (this.eventBus) {
      this.eventBus.publish({
