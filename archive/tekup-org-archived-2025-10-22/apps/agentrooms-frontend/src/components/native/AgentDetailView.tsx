...
const logger = createLogger('apps-agentrooms-frontend-src-c');
  ...
  const loadAgentHistory = useCallback(async () => {
    ...
    try {
      ...
      logger.info(`📚 Loading history for agent: ${agent.name} (${agent.id})`);

      // Get projects for this specific agent
      const agentProjects = await remoteHistory.fetchAgentProjects(agent.apiEndpoint);
      logger.info(`📁 Found ${agentProjects.length} projects for agent ${agent.name}`);
      ...
      for (const project of agentProjects) {
        try {
          logger.info(`📖 Loading conversations from project: ${project.path}`);
          const projectHistories = await remoteHistory.fetchAgentHistories(
            agent.apiEndpoint, 
            project.encodedName,
            agent.id // Pass agent ID for filtering
          );
          logger.info(`💬 Found ${projectHistories.length} conversations in project ${project.path}`);
          allAgentConversations.push(...projectHistories);
        } catch (projectError) {
          logger.warn(`Failed to load project ${project.path} for agent ${agent.name}:`, projectError);
        }
      }
      ...
      logger.info(`✅ Loaded ${allAgentConversations.length} total conversations for agent ${agent.name}`);
      
    } catch (err) {
      logger.error("❌ Failed to load agent conversations:", err);
  ...
  const handleHistoryConversationSelect = useCallback(async (sessionId: string) => {
    try {
      ...
      try {
        ...
        logger.info(`🔍 Searching for session ${sessionId} in ${agentProjects.length} projects`);
        ...
        for (const project of agentProjects) {
          try {
            ...
            if (conversation) {
              ...
              logger.info(`✅ Found session in project: ${project.path}`);
              break;
            }
          } catch {
        ...
        logger.warn("Could not search agent projects for session:", error);
      }
      
      // Continue if no project found
      if (!foundProject) {
        logger.warn("No matching project found for conversation");
        setHistoryError("Could not find the project for this conversation");
        return;
      }
      
      // Get the conversation directly from remote agent
      logger.info(`🔍 Loading conversation ${sessionId} from project ${foundProject}`);
      ...
      if (conversation && conversation.messages && conversation.messages.length > 0) {
        logger.info("📚 Loading historical conversation:", {
          sessionId,
          agentId,
        ...
        logger.info("✅ Historical conversation loaded successfully");
      } else {
        logger.warn("⚠️ No messages found in historical conversation");
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      if (!agent) {
        logger.info("❌ CRITICAL ERROR - Agent not found for ID:", agentId);
        return;
      }

      // Debug OAuth authentication state
      logger.info("🔐 [AUTH DEBUG] OAuth session state:", claudeSession ? "✅ AUTHENTICATED" : "❌ NOT AUTHENTICATED");
      if (claudeSession) {
        logger.info("🔐 [AUTH DEBUG] OAuth user:", claudeSession.account?.email_address);
        logger.info("🔐 [AUTH DEBUG] OAuth expires:", new Date(claudeSession.expiresAt).toISOString());
        logger.info("🔐 [AUTH DEBUG] Including claudeAuth in request");
      } else {
        logger.info("🔐 [AUTH DEBUG] No OAuth session - request will use system credentials");
      }
  ...
  const handleExecutePlan = useCallback(async (steps: ExecutionStep[]) => {
    logger.info("Executing plan with", steps.length, "steps");
    ...
    const executeStepsRecursively = async (remainingSteps: ExecutionStep[]) => {
      ...
      if (executableSteps.length === 0) {
        logger.info("No more executable steps found");
        return;
      }
      
      // Execute all executable steps in parallel
      logger.info(`Executing ${executableSteps.length} steps:`, executableSteps.map(s => s.id));
