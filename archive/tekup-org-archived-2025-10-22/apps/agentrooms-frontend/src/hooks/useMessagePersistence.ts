...
const logger = createLogger('apps-agentrooms-frontend-src-h');
  ...
  useEffect(() => {
    ...
    logger.info('🔄 Loading conversation from Electron storage:', sessionId);
    ...
    window.electronAPI!.storage.loadConversation(sessionId)
      .then((result) => {
        if (result.success && result.data && result.data.messages && result.data.messages.length > 0) {
          logger.info('📖 Loaded messages from storage:', result.data.messages.length);
          ...
          logger.info('📝 No existing conversation found for session:', sessionId);
          hasLoadedRef.current = true;
        }
      })
      .catch((error) => {
  ...
  const saveMessages = useCallback(() => {
    logger.info('🔍 saveMessages called:', {
    ...
    if (messagesStringified === lastSavedStringified) {
      logger.info('⏭️ Skipping save: messages unchanged');
      return;
    }

    logger.info('💾 Saving conversation to Electron storage:', {
      sessionId,
      messageCount: messages.length,
      agentId,
  ...
  useEffect(() => {
    ...
    logger.info('🔄 Messages changed, scheduling save...', { 
      messageCount: messages.length, 
      sessionId,
  ...
  const listConversations = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        logger.error('❌ Failed to list conversations:', result.error);
        return [];
      }
    } catch (error) {
  ...
  const clearConversation = useCallback(async (targetSessionId?: string) => {
    ...
    try {
      ...
      if (result.success) {
        logger.info('🗑️ Cleared conversation:', sessionToClear);
