...
const logger = createLogger('apps-agentrooms-backend-app-ts');
  ...
  // New: expose registered agents for frontend sync
  app.get("/api/agents", (c) => {
    try {
      const agents = globalRegistry.getAllAgents().map(a => ({
        id: a.id,
        name: a.name,
      ...
      return c.json({ agents: [], error: "Failed to list agents" }, 500);
    }
  });
...
function initializeMultiAgentSystem(config: AppConfig): void {
  ...
  globalImageHandler.initialize().catch(error => {
    logger.warn("Failed to initialize image handler:", error);
  });
  ...
  if (config.debugMode) {
    logger.debug("[Multi-Agent] Initialized with agents:", 
