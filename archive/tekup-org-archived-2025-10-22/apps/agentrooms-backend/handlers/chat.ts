...
function shouldUseOrchestrator(message: string, availableAgents?: Array<{id: string; name: string; description: string; isOrchestrator?: boolean}>): boolean {
  ...
  logger.debug(`[DEBUG] shouldUseOrchestrator check:`, {
  ...
  if (orchestratorProvider?.id !== "anthropic") {
    logger.debug(`[DEBUG] Orchestrator provider is not 'anthropic', got:`, orchestratorProvider?.id);
    return false;
  }
  
  // Use orchestrator if there are available agents and:
  // 1. Multiple agent mentions, OR
  // 2. Single mention of @orchestrator, OR  
  // 3. No mentions (general orchestration)
  if (availableAgents && availableAgents.length > 0) {
    const mentionMatches = message.match(/@(\w+(?:-\w+)*)/g);
    
    if (!mentionMatches) {
      // No mentions - use orchestration for general queries
      const result = true;
      logger.debug(`[DEBUG] shouldUseOrchestrator result (no mentions):`, { result });
      return result;
    }
    
    if (mentionMatches.length > 1) {
      // Multiple mentions - use orchestration
      const result = true;
      logger.debug(`[DEBUG] shouldUseOrchestrator result (multiple mentions):`, {
        mentionMatches,
    ...
    // Single mention - check if it's @orchestrator
    const singleMention = mentionMatches[0].substring(1); // Remove @
    const isOrchestratorMention = singleMention === "orchestrator";
    
    logger.debug(`[DEBUG] shouldUseOrchestrator result (single mention):`, {
      mentionMatches,
      singleMention,
      isOrchestratorMention,
      result: isOrchestratorMention
    });
    
    return isOrchestratorMention;
  }
  
  logger.debug(`[DEBUG] No available agents or empty array`);
  return false;
}
...
async function* executeAgentHttpRequest(
  ...
  try {
    ...
    if (debugMode) {
      logger.debug(`[DEBUG] Making HTTP request to agent ${agent.id} at ${agent.apiEndpoint}`);
      logger.debug(`[DEBUG] Request payload (OAuth masked):`, {
        ...agentChatRequest,
    ...
    if (!response.ok) {
      ...
      if (debugMode) {
        logger.error(`[DEBUG] Agent HTTP request failed with status ${response.status}:`);
      ...
      while (true) {
        ...
        for (const line of lines) {
          try {
            ...
            if (debugMode) {
              logger.debug(`[DEBUG] Agent response:`, JSON.stringify(streamResponse, null, 2));
            }
            ...
            if (debugMode) {
              logger.debug(`[DEBUG] Failed to parse line: ${line}`, parseError);
            }
    ...
    if (debugMode) {
      logger.error(`[DEBUG] Agent HTTP request failed:`, error);
    }
...
async function* executeOrchestratorWorkflow(
  ...
  try {
    ...
    let workerAgents = availableAgents?.filter(agent => !agent.isOrchestrator) || [];
    
    // If no worker agents are available, provide fallback agents
    ...
    if (debugMode) {
      logger.debug(`[DEBUG] Orchestrator using API Key authentication`);
      logger.debug(`[DEBUG] API Key available:`, !!apiKey);
    }
...
4. CRITICAL: You MUST only use agent IDs from the Available Agents list below
5. NEVER use placeholder values like "<UNKNOWN>" or make up agent IDs
6. If the request is simple and can be handled by a single agent, create a single step

Available Agents:
${agentDescriptions}

IMPORTANT: Always use the orchestrate_execution tool to create step-by-step plans. Each step must specify a valid agent ID from the list above.`;
    ...
    for await (const chunk of stream) {
      if (debugMode) {
        logger.debug("[DEBUG] Anthropic API Chunk:");
        logger.debug(JSON.stringify(chunk, null, 2));
        logger.debug("---");
      }
      ...
      if (chunk.type === "message_start") {
        ...
        if (lastContent && lastContent.type === "tool_use") {
          if (debugMode) {
            logger.debug("Content block stopped, input type:", typeof lastContent.input);
            logger.debug("Input length:", lastContent.input?.length || 0);
            logger.debug("First 100 chars:", typeof lastContent.input === "string" ? lastContent.input.substring(0, 100) : "Not a string");
          }
          ...
          if (typeof lastContent.input === "string" && lastContent.input.trim()) {
            try {
              ...
              if (debugMode) {
                ...
                logger.error("Raw input:", lastContent.input);
              }
            }
          }
        }
    ...
    if (debugMode) {
      logger.error("Anthropic API execution failed:", error);
    }
    yield {
      type: "error",
      error: error instanceof Error ? error.message : String(error),
...
async function* executeClaudeCommand(
  ...
  try {
    ...
    try {
      if (debugMode) {
        logger.info("[DEBUG] Starting authentication setup...");
      }
      ...
      if (debugMode) {
        logger.info("[DEBUG] Credentials file written, preparing auth environment...");
      }
      ...
      if (debugMode && Object.keys(authEnv).length > 0) {
        logger.info("[DEBUG] Using Claude OAuth authentication");
        logger.info("[DEBUG] Auth environment variables:", Object.keys(authEnv));
      }
    } catch (authError) {
      logger.warn("[WARN] Failed to prepare Claude auth environment:", authError);
      if (debugMode) {
        logger.debug("[DEBUG] Auth error details:", authError);
      }
    ...
    if (claudeAuth?.accessToken) {
      ...
      if (debugMode) {
        logger.info("[DEBUG] Set CLAUDE_CODE_OAUTH_TOKEN and cleared API key env vars");
        logger.info("[DEBUG] OAuth token length:", claudeAuth.accessToken.length);
      }
    }
    ...
    try {
      for await (const sdkMessage of query({
        ...
        if (debugMode) {
          logger.debug("[DEBUG] Claude SDK Message:");
          logger.debug(JSON.stringify(sdkMessage, null, 2));
          logger.debug("---");
        }
...
const logger = createLogger('apps-agentrooms-backend-handle');
  ...
  if (chatRequest.claudeAuth) {
    try {
      if (debugMode) {
        logger.debug("[DEBUG] Using OAuth credentials from request");
        logger.debug("[DEBUG] OAuth user:", chatRequest.claudeAuth.account?.email_address);
        logger.debug("[DEBUG] OAuth expires:", new Date(chatRequest.claudeAuth.expiresAt));
      }
      ...
      if (debugMode) {
        logger.debug("[DEBUG] OAuth credentials written successfully for Claude Code execution");
      }
    } catch (error) {
      logger.error("[ERROR] Failed to write OAuth credentials:", error);
      // Don't fail the request, fall back to system credentials
    }
  } else if (debugMode) {
    logger.debug("[DEBUG] No OAuth credentials provided, using system credentials");
  }
        ...
        if (shouldUseOrchestrator(chatRequest.message, chatRequest.availableAgents)) {
          ...
          if (mentionMatches && mentionMatches.length === 1 && chatRequest.availableAgents) {
            ...
            if (mentionedAgent) {
              ...
              if (debugMode) {
                logger.debug(`[DEBUG] Single agent ${mentionedAgentId} mentioned, making HTTP request to ${mentionedAgent.apiEndpoint}`);
              }
