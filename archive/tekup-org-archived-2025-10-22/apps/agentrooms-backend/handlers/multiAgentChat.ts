...
async function* executeMultiAgentChat(
  ...
  try {
    ...
    if (debugMode) {
      logger.debug("[Multi-Agent] Processing request:", {
    ...
    if (mentionMatches && mentionMatches.length === 1) {
      ...
      if (debugMode) {
        logger.debug(`[Multi-Agent] Single agent mentioned: ${mentionedAgentId}`);
      }
...
async function* handleScreenCapture(
  ...
  try {
    if (debugMode) {
      logger.debug(`[Multi-Agent] Handling screen capture for agent: ${agentId}`);
    }
