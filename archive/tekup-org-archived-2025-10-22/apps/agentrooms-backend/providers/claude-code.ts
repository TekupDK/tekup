...
const logger = createLogger('apps-agentrooms-backend-provid');
  ...
  async* executeChat(
    ...
    try {
      ...
      if (debugMode) {
        logger.debug(`[Claude Code] Executing chat request:`, {
      ...
      try {
        ...
        if (debugMode && Object.keys(authEnv).length > 0) {
          logger.debug("[Claude Code] Using OAuth authentication");
        }
      } catch (authError) {
        logger.warn("[Claude Code] Failed to prepare auth environment:", authError);
      ...
      try {
        ...
        for await (const sdkMessage of query({
          ...
          if (debugMode) {
            logger.debug(`[Claude Code] SDK Message:`, {
      ...
      if (error instanceof AbortError) {
        ...
        if (options.debugMode) {
          logger.error(`[Claude Code] Chat execution failed:`, error);
        }
