/**
 * Create Plan Modal (Category B - Sprint 1)
 * 
 * Modal for creating new cleaning plans with task builder
 * Features:
 * - Plan details form (name, service type, frequency)
 * - Task builder with drag-drop support
 * - Default task templates by service type
 * - Price calculator
 * - Square meters and address input
 */

import { useState, useEffect } from 'react';
import { X, Plus, Trash2, GripVertical, CheckSquare } from 'lucide-react';
...
const CreatePlanModal = ({ isOpen, onClose, onSuccess, customerId = 'default' }: CreatePlanModalProps) => {
  ...
  useEffect(() => {
    // Load default tasks when service type changes
    if (templates[serviceType]) {
      setTasks(templates[serviceType]);
    }
  }, [serviceType, templates]);

  const fetchTemplates = async () => {
    try {
      const response = await fetch(`${API_URL}/api/cleaning-plans/templates/tasks`);
  ...
  const calculatePrice = () => {
    ...
    const hourlyRate = 349; // Default rate
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/cleaning-plans`, {
        ...
        body: JSON.stringify({
          ...
          address,
          notes,
          tasks: tasks.map(({ name, description, category, estimatedTime, isRequired, pricePerTask }) => ({
            name,
            description,
            category,
            estimatedTime,
      ...
      if (data.success) {
        ...
        alert(`Fejl: ${data.error || 'Kunne ikke oprette plan'}`);
      }
    } catch (error) {
      console.error('Error creating plan:', error);
      alert('Der opstod en fejl ved oprettelse af planen');
    } finally {
      setLoading(false);
    }
  };
