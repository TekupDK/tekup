...
export class ChatSessionsService {
  ...
  async getSession(sessionId: string, userId: string): Promise<ChatSession> {
    try {
      ...
      if (error) {
        ...
        throw new BadRequestException(`Failed to get chat session: ${error.message}`);
      }
  ...
  async updateSession(
    ...
    try {
      ...
      if (error) {
        throw new BadRequestException(`Failed to update chat session: ${error.message}`);
      }
  ...
  async deleteSession(sessionId: string, userId: string): Promise<void> {
    try {
      ...
      if (error) {
        throw new BadRequestException(`Failed to delete chat session: ${error.message}`);
      }
  ...
  async addMessage(
    ...
    try {
      ...
      if (error) {
        throw new BadRequestException(`Failed to add chat message: ${error.message}`);
      }
  ...
  async getSessionMessages(
    ...
    try {
      ...
      if (error) {
        throw new BadRequestException(`Failed to get session messages: ${error.message}`);
      }
  ...
  async getSessionAnalytics(
    ...
    try {
      ...
      if (sessionError) {
        throw new BadRequestException(`Failed to get session analytics: ${sessionError.message}`);
      }
      ...
      if (messageError) {
        throw new BadRequestException(`Failed to get message analytics: ${messageError.message}`);
      }
