...
export class AuditService {
  ...
  async generateAuditReport(
    ...
    details: {
      ...
      securityEventsByType: Array<{ eventType: string; count: number; severity: string }>;
    };
  }> {
    try {
      ...
      const securityEventsByType = securityEvents.reduce((acc, event) => {
        ...
        acc[key].count++;
        return acc;
      }, {} as Record<string, { eventType: string; count: number; severity: string }>);
  ...
  async logUserLogin(organizationId: string, userId: string, ipAddress?: string, userAgent?: string): Promise<void> {
    await this.logAction({
  ...
  async logUserLogout(organizationId: string, userId: string, ipAddress?: string): Promise<void> {
    await this.logAction({
      organization_id: organizationId,
      user_id: userId,
      action: 'user_logout',
  ...
  async logFailedLogin(organizationId: string, email: string, ipAddress?: string, userAgent?: string): Promise<void> {
