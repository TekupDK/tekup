...
export class QualityChecklistsService extends BaseService<QualityChecklist> {
  ...
  async create(createDto: CreateQualityChecklistDto, organizationId: string): Promise<QualityChecklist> {
    ...
    if (error) {
      throw new BadRequestException(`Failed to create quality checklist: ${error.message}`);
    }

    return data;
  }

  async getByServiceType(serviceType: ServiceType, organizationId: string): Promise<QualityChecklist | null> {
    const { data, error } = await this.supabaseService.client
      .from('quality_checklists')
      .select('*')
      .eq('organization_id', organizationId)
    ...
    if (error && error.code !== 'PGRST116') {
      throw new BadRequestException(`Failed to get checklist: ${error.message}`);
    }

    return data;
  }

  async createNewVersion(checklistId: string, organizationId: string, updates: Partial<QualityChecklist>): Promise<QualityChecklist> {
    ...
    if (error) {
      throw new BadRequestException(`Failed to create new checklist version: ${error.message}`);
    }

    return data;
  }

  async getChecklistVersions(serviceType: ServiceType, organizationId: string): Promise<QualityChecklist[]> {
    const { data, error } = await this.supabaseService.client
      .from('quality_checklists')
      .select('*')
      .eq('organization_id', organizationId)
    ...
    if (error) {
      throw new BadRequestException(`Failed to get checklist versions: ${error.message}`);
    }

    return data || [];
  }

  async duplicateChecklist(checklistId: string, organizationId: string, newServiceType: ServiceType, newName: string): Promise<QualityChecklist> {
    ...
    if (error) {
      throw new BadRequestException(`Failed to duplicate checklist: ${error.message}`);
    }

    return data;
  }

  async validateChecklistItems(items: ChecklistItem[]): Promise<{ isValid: boolean; errors: string[] }> {
    const errors: string[] = [];
  ...
  async getChecklistAnalytics(organizationId: string): Promise<any> {
    ...
    if (error) {
      throw new BadRequestException(`Failed to get checklist analytics: ${error.message}`);
    }
    ...
    if (assessments && assessments.length > 0) {
      ...
      assessments.forEach(assessment => {
        analytics.score_distribution[assessment.overall_score as keyof typeof analytics.score_distribution]++;
      });
  ...
  async initializeDefaultChecklists(organizationId: string): Promise<QualityChecklist[]> {
    ...
    for (const checklistData of defaultChecklists) {
      try {
        const checklist = await this.create(checklistData as CreateQualityChecklistDto, organizationId);
