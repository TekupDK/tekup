...
let RealtimeGateway = RealtimeGateway_1 = class RealtimeGateway {
    ...
    handleJobStatusUpdate(client, data) {
    ...
    handleCustomerMessage(client, data) {
        if (!client.userId || !client.organizationId)
            return;
        this.server
    ...
    broadcastToRole(organizationId, role, event, data) {
        const targetUsers = Array.from(this.connectedUsers.values()).filter((socket) => socket.organizationId === organizationId && socket.userRole === role);
