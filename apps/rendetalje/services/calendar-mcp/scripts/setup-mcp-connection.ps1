# RenOS Calendar MCP - MCP Connection Setup
...
Write-Host "=== Step 3: Testing Tool Functionality ===" -ForegroundColor Magenta
try {
    $testData = @{
        date = "2025-01-20"
    ...
    $response = Invoke-RestMethod "http://localhost:3001/validate-booking" -Method POST -Body $testData -ContentType "application/json"
    Write-Host "✓ Tool functionality working" -ForegroundColor Green
    Write-Host "  Test result: $($response.valid)" -ForegroundColor White
} catch {
...
Write-Host "=== Step 4: Creating MCP Configuration ===" -ForegroundColor Magenta
...
Write-Host "=== Step 5: Creating Environment File ===" -ForegroundColor Magenta
$envContent = @"
...
$endpoints = @(
    ...
    @{ Name = "Tools List"; Url = "http://localhost:3001/tools" },
    @{ Name = "Validate Booking"; Url = "http://localhost:3001/validate-booking"; Method = "POST" }
)
...
foreach ($endpoint in $endpoints) {
    try {
        if ($endpoint.Method -eq "POST") {
            ...
            $response = Invoke-RestMethod $endpoint.Url -Method POST -Body $testData -ContentType "application/json"
        } else {
            $response = Invoke-RestMethod $endpoint.Url
        }
