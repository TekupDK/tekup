...
CREATE TABLE jobs (
  ...
  job_number TEXT UNIQUE NOT NULL, -- Human-readable job number
  service_type service_type NOT NULL,
  status job_status NOT NULL DEFAULT 'scheduled',
  scheduled_date TIMESTAMPTZ NOT NULL,
  estimated_duration INTEGER NOT NULL, -- minutes
  actual_duration INTEGER,
  location JSONB NOT NULL, -- Address and GPS coordinates
  special_instructions TEXT,
  checklist JSONB DEFAULT '[]',
  photos JSONB DEFAULT '[]',
  customer_signature TEXT, -- Base64 encoded signature
  quality_score INTEGER CHECK (quality_score >= 1 AND quality_score <= 5),
  profitability JSONB, -- Cost breakdown and profit calculation
  recurring_job_id UUID, -- Link to recurring job template
  parent_job_id UUID REFERENCES jobs(id), -- For rescheduled jobs
...
CREATE TABLE job_assignments (
  ...
  role TEXT DEFAULT 'cleaner', -- 'lead', 'cleaner', 'supervisor'
...
CREATE TABLE time_entries (
  ...
  notes TEXT,
  location JSONB, -- GPS coordinates for verification
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE customer_messages (
  ...
  sender_type TEXT NOT NULL, -- 'customer', 'employee', 'system'
  message_type TEXT DEFAULT 'text', -- 'text', 'photo', 'file'
...
CREATE TABLE chat_sessions (
  ...
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  organization_id UUID REFERENCES organizations(id) ON DELETE CASCADE,
  context TEXT NOT NULL, -- 'owner', 'employee', 'customer'
  title TEXT,
...
CREATE TABLE job_quality_assessments (
  ...
  completed_items JSONB NOT NULL, -- Completed checklist items with photos
...
CREATE TABLE notifications (
  ...
  organization_id UUID REFERENCES organizations(id) ON DELETE CASCADE,
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  type TEXT NOT NULL, -- 'job_assigned', 'job_completed', 'payment_received', etc.
  title TEXT NOT NULL,
