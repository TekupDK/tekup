...
describe('ApiClient', () => {
  ...
  describe('Token Refresh', () => {
    it('should retry request after token refresh on 401', async () => {
      ...
      (fetch as jest.Mock)
        .mockImplementationOnce(() => mockResponse(401, { message: 'Unauthorized' }))
        .mockImplementationOnce(() => mockResponse(200, { accessToken: 'new-token' }))
    ...
    it('should logout if token refresh fails', async () => {
      (fetch as jest.Mock)
        .mockImplementationOnce(() => mockResponse(401, { message: 'Unauthorized' }))
        .mockImplementationOnce(() => mockResponse(401, { message: 'Invalid token' }));
