...
test.describe("Dashboard Page", () => {
  ...
  test("should display real numbers from backend", async ({ page }) => {
    ...
    const activeJobs = await page
      ...
      .textContent();
    const pendingJobs = await page
      .locator('[data-testid="pending-jobs"]')
      .textContent();
    const completedJobs = await page
      .locator('[data-testid="completed-jobs"]')
      .textContent();
    const totalCustomers = await page
      .locator('[data-testid="total-customers"]')
      .textContent();
  ...
  test("should navigate to customers page from quick actions", async ({
    page,
  }) => {
  ...
  test("should refresh stats when navigating back from other pages", async ({
    page,
  }) => {
    // Get initial stats
    const initialActiveJobs = await page
      .locator('[data-testid="active-jobs"]')
      .textContent();
    ...
    const newActiveJobs = await page
      .locator('[data-testid="active-jobs"]')
      .textContent();
    expect(newActiveJobs).toBeDefined();
  });

  test("should display stat cards with correct emoji icons", async ({
    page,
  }) => {
  ...
  test("should be responsive on mobile viewport", async ({ page }) => {
    ...
    const statCards = page.locator(
      '[data-testid*="-jobs"], [data-testid="total-customers"]'
    );
