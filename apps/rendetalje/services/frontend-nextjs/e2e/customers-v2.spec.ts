...
test.describe('Customers Page v2', () => {
  ...
  test('should create new customer with full address', async ({ page }) => {
    ...
    // Should show toast
    await expect(page.getByText(/kunde oprettet/i)).toBeVisible({ timeout: 5000 });
  ...
  test('should display customer cards with email and phone links', async ({ page }) => {
    await page.waitForTimeout(2000);
  ...
  test('should edit customer details', async ({ page }) => {
    ...
    if (await editButton.isVisible()) {
      ...
      // Should show toast
      await expect(page.getByText(/kunde opdateret/i)).toBeVisible({ timeout: 5000 });
    }
  });
  ...
  test('should delete customer with confirmation', async ({ page }) => {
    ...
    if (await customerToDelete.isVisible()) {
      ...
      // Should show toast
      await expect(page.getByText(/kunde slettet/i)).toBeVisible({ timeout: 5000 });
  ...
  test('should handle empty customers list', async ({ page }) => {
    ...
    await page.route('**/customers', route => route.fulfill({
      status: 200,
  ...
  test('should display customers in 3-column grid on desktop', async ({ page }) => {
  ...
  test('should show loading state while fetching customers', async ({ page }) => {
    await page.goto('http://localhost:3001/customers');
