...
test.describe("Jobs Page v2", () => {
  ...
  test("should display jobs page with filter and search", async ({ page }) => {
    ...
    const searchInput = page
      .locator('input[type="text"], input[placeholder*="søg" i]')
      .first();
  ...
  test("should filter jobs by status", async ({ page }) => {
    ...
    const pendingJobsCount = await page
  ...
  test("should validate required fields when creating job", async ({
    page,
  }) => {
    await page.click('button:has-text("Opret Opgave")');
    await page.waitForSelector('[role="dialog"]');
  ...
  test("should edit job title", async ({ page }) => {
    ...
    if (await editButton.isVisible()) {
      ...
      // Should show toast
      await expect(page.getByText(/job opdateret/i)).toBeVisible({
        timeout: 5000,
      });
    }
  });
