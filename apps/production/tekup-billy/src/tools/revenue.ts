  ...
  try {
    ...
    const responseData = {
      ...
      revenue: {
        ...
        summary: {
          averageInvoiceValue: revenueData.paidInvoices > 0
            ? Math.round(revenueData.totalRevenue / revenueData.paidInvoices * 100) / 100
            : 0,
          totalInvoices: revenueData.paidInvoices + revenueData.pendingInvoices + revenueData.overdueInvoices,
          paymentRate: revenueData.paidInvoices > 0
            ? Math.round((revenueData.paidInvoices / (revenueData.paidInvoices + revenueData.pendingInvoices + revenueData.overdueInvoices)) * 100)
            : 0,
        },
      },
    };
