...
const listInvoicesSchema = z.object({
  ...
  state: z.enum(['draft', 'approved', 'voided']).optional().describe('Invoice state filter (Billy API only supports: draft, approved, voided)'),
...
const createInvoiceSchema = z.object({
  ...
  lines: z.array(z.object({
    ...
    productId: z.string().describe('Product ID (required by Billy API - use list_products to find valid IDs)'),
...
export async function listInvoices(client: BillyClient, args: unknown) {
  ...
  try {
    ...
    if (!invoices || !Array.isArray(invoices)) {
      ...
      throw new Error('Invalid response format from Billy API - expected array of invoices');
    }
...
export async function createInvoice(client: BillyClient, args: unknown) {
  ...
  try {
    ...
    for (const line of invoiceData.lines) {
      if (!line.productId) {
        throw new Error('All invoice lines must have a productId. Use list_products to find valid product IDs.');
      }
    }
    ...
    // Add null checks
    if (!invoice) {
      log.error('Invalid invoice creation response from Billy API', null, { invoice });
      throw new Error('Failed to create invoice - invalid response from Billy API');
    }
    ...
    const successMessage = invoice.state === 'draft' 
...
export async function markInvoicePaid(client: BillyClient, args: unknown) {
  const startTime = Date.now();
  try {
    const { invoiceId, paymentDate, amount } = markInvoicePaidSchema.parse(args);
    ...
    const invoice = await client.markInvoicePaid(invoiceId, paymentDate, amount);
