 ...
 * for standalone cloud deployment on Render.com, AWS, etc.
 * 
 * Architecture:
...
import { z } from 'zod';
import { initializeRedis, getRedisClient, isRedisEnabled, checkRedisHealth, RedisSessionStore } from './utils/redis-client.js';
import { createCircuitBreaker } from './utils/circuit-breaker.js';
import { getHealthMonitor, initializeHealthMonitoring } from './monitoring/health-monitor.js';
import { createAuditor } from './middleware/audit-logger.js';

// Import Streamable HTTP transport (new MCP standard 2025-03-26)
import { handleMcpPost, handleMcpGet, handleMcpDelete } from './mcp-streamable-transport.js';
...
// Trust proxy for Render.com (required for rate limiting with X-Forwarded-For)
...
const limiter = (() => {
  ...
  if (redisClient && isRedisEnabled()) {
    ...
    log.warn('Rate limiter: Using in-memory (not distributed across instances)');
    return rateLimit({
      windowMs: 15 * 60 * 1000,
      max: 100,
      validate: { trustProxy: false }, // Disable trust proxy validation (we handle it explicitly)
...
const authenticate = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!apiKey || apiKey !== validApiKey) {
        return res.status(401).json({
            ...
            message: 'Please provide a valid X-API-Key header or ?apiKey=<token> query parameter'
        });
    }
...
app.post('/', (req: Request, res: Response) => {
    ...
    Promise.resolve(handleMcpPost(req, res))
        .catch(err => {
...
app.get('/version', (req: Request, res: Response) => {
    ...
    // Get git SHA from environment (set by Render)
    const gitSHA = process.env.RENDER_GIT_COMMIT || 'unknown';
...
app.post('/mcp/messages', async (req: Request, res: Response): Promise<void> => {
 ...
 * GET /api/v1/tools
 */
app.get(`/api/${API_VERSION}/tools`, authenticate, (req: Request, res: Response) => {
 ...
 * Body: { arguments: { ... } }
 */
app.post(`/api/${API_VERSION}/tools/:toolName`, authenticate, async (req: Request, res: Response): Promise<void> => {
    const { toolName } = req.params;
    const args = req.body.arguments || req.body;

    const startTime = Date.now();
    const auditContext = getAuditContext(req);
    const requestAuditor = supabaseAvailable && organizationId
        ? createAuditor(
            organizationId,
            auditContext.userId,
    ...
    try {
        ...
        const executeTool = requestAuditor
            ? requestAuditor.wrap(toolName!, auditAction, async (wrappedArgs: unknown) => {
                return await toolFunction(billyClient, wrappedArgs);
            })
            : async (wrappedArgs: unknown) => toolFunction(billyClient, wrappedArgs);
 ...
 * }
 */
app.post(`/api/${API_VERSION}/tools/batch`, authenticate, async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        for (const toolCall of tools) {
            ...
            try {
                ...
                const executeTool = requestAuditor
                    ? requestAuditor.wrap(tool, auditAction, async (wrappedArgs: unknown) => {
                        return await toolFunction(billyClient, wrappedArgs);
                    })
                    : async (wrappedArgs: unknown) => toolFunction(billyClient, wrappedArgs);
 ...
 */
app.post(`/api/${API_VERSION}/call`, authenticate, async (req: Request, res: Response) => {
...
function getToolCategory(toolName: string): string {
    ...
    if (toolName.includes('test') || toolName.includes('generate')) return 'testing';
    return 'general';
}
...
function setupMcpTools(server: McpServer): void {
    ...
    server.tool(
        ...
        {
            state: z.enum(['draft', 'approved', 'sent', 'paid', 'cancelled']).optional().describe('Filter by invoice state'),
    ...
    server.tool(
        ...
        'Update an existing invoice',
        {
            ...
            })).optional().describe('Invoice lines')
        },
    ...
    server.tool(
        ...
        {
            businessType: z.string().describe('Business type (e.g., retail, consulting)')
        },
    ...
    server.tool(
        ...
        {
            endpoint: z.string().optional().describe('Endpoint to test (organization, contacts, products, invoices)')
        },
    ...
    server.tool(
        ...
        {
            includeDiagnostics: z.boolean().optional().describe('Include timing and environment info')
        },
    ...
    try {
        ...
        app.listen(PORT, () => {
            log.info('Tekup-Billy MCP HTTP Server started', {
                ...
                mcpSse: `http://localhost:${PORT}/mcp`,
...
function getAuditContext(req: Request) {
    ...
    return {
        userId,
        ipAddress: req.ip,
        userAgent: getHeaderValue(req.headers['user-agent'] as string | string[] | undefined)
    };
}
