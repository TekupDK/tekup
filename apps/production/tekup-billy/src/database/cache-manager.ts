...
import type { BillyContact, BillyInvoice, BillyProduct } from "../types.js";
import { log } from "../utils/logger.js";
import {
  getRedisClusterManager,
  isRedisEnabled,
  RedisClusterManager,
} from "./redis-cluster-manager.js";
import {
  getBillyApiKey,
  getCachedData,
  invalidateCache,
...
export class CacheManager {
  ...
  constructor(
    ...
    cacheTTL: number = DEFAULT_CACHE_TTL.SUPABASE_MINUTES
  ) {
      ...
      try {
        ...
        if (redisResult !== null) {
          ...
          this.localCache.set(
            key,
            redisResult,
            DEFAULT_CACHE_TTL.LOCAL_SECONDS
          );
          return redisResult;
        }
      } catch (error) {
        log.warn("Redis cache get failed, falling back to Supabase", {
          key,
          error,
        });
      }
    ...
    if (this.supabaseEnabled) {
      try {
        ...
        if (keyParts.length >= 4) {
          ...
          const id: string | undefined = keyParts[3];
          const table = this.getSupabaseTable(type);

          if (table && id) {
            const supabaseResult = await getCachedData<T>(
              table,
              id,
              this.organizationId
            );
            if (supabaseResult !== null) {
              ...
              if (this.redisManager) {
                await this.redisManager.set(
                  key,
                  supabaseResult,
                  DEFAULT_CACHE_TTL.REDIS_SECONDS
                );
              }
   ...
   */
  private async invalidateFromCache(
    key: string,
    type: string,
    id: string
  ): Promise<void> {
  ...
  async getInvoice(
    ...
    try {
      ...
      if (cached) {
        ...
        log.debug("Cache hit for invoice", {
          invoiceId,
          tier: cacheTier,
          duration,
        });
  ...
  async clearAllCache(): Promise<void> {
    try {
      ...
      log.info("Cache cleared for organization", {
        organizationId: this.organizationId,
      });
    } catch (error) {
  ...
  async warmCache(): Promise<void> {
    try {
      ...
      for (const invoice of recentInvoices.slice(0, 50)) {
   ...
   */
  setCacheStrategy(
    strategy: "redis-first" | "supabase-first" | "local-only"
  ): void {
