...
interface ActionDef<P extends z.ZodTypeAny, R = unknown> {
  ...
  risk: RiskLevel
  paramsSchema: P
  dryRun?: (params: z.infer<P>, ctx: Ctx) => Promise<{ ok: true; diff?: string; affectedCount?: number; recipients?: string[]; estimate?: number } | { ok: false; reason: string }>
  execute: (params: z.infer<P>, ctx: Ctx & { idempotencyKey: string }) => Promise<R>
}
```
