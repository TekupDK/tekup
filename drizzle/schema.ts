import { pgTable, pgSchema, serial, integer, varchar, timestamp, text, unique, boolean, jsonb, numeric, index, uniqueIndex } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const fridayAi = pgSchema("friday_ai");
export const calendarStatusInFridayAi = fridayAi.enum("calendar_status", ['confirmed', 'tentative', 'cancelled'])
export const customerInvoiceStatusInFridayAi = fridayAi.enum("customer_invoice_status", ['draft', 'approved', 'sent', 'paid', 'overdue', 'voided'])
export const emailPipelineStageInFridayAi = fridayAi.enum("email_pipeline_stage", ['needs_action', 'venter_pa_svar', 'i_kalender', 'finance', 'afsluttet'])
export const invoiceStatusInFridayAi = fridayAi.enum("invoice_status", ['draft', 'sent', 'paid', 'overdue', 'cancelled'])
export const leadStatusInFridayAi = fridayAi.enum("lead_status", ['new', 'contacted', 'qualified', 'proposal', 'won', 'lost'])
export const messageRoleInFridayAi = fridayAi.enum("message_role", ['user', 'assistant', 'system'])
export const taskPriorityInFridayAi = fridayAi.enum("task_priority", ['low', 'medium', 'high', 'urgent'])
export const taskStatusInFridayAi = fridayAi.enum("task_status", ['todo', 'in_progress', 'done', 'cancelled'])
...
export const calendarEventsInFridayAi = fridayAi.table("calendar_events", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	...
	unique("calendar_events_googleEventId_key").on(table.googleEventId),
]);
...
export const emailsInFridayAi = fridayAi.table("emails", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
...
export const invoicesInFridayAi = fridayAi.table("invoices", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	invoiceNumber: varchar({ length: 50 }),
	...
	unique("invoices_invoiceNumber_key").on(table.invoiceNumber),
]);

export const userCredentialsInFridayAi = fridayAi.table("user_credentials", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	...
	unique("user_credentials_userId_provider_key").on(table.userId, table.provider),
]);

export const billyRateLimitInFridayAi = fridayAi.table("billy_rate_limit", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	...
	unique("billy_rate_limit_userId_key").on(table.userId),
]);

export const billyApiCacheInFridayAi = fridayAi.table("billy_api_cache", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	...
	unique("billy_api_cache_userId_cacheKey_key").on(table.userId, table.cacheKey),
]);
...
export const emailAnalysisInFridayAi = fridayAi.table("email_analysis", {
	id: serial().primaryKey().notNull(),
	emailId: integer().notNull(),
	...
	unique("email_analysis_emailId_key").on(table.emailId),
]);
...
export const userSettingsInFridayAi = fridayAi.table("user_settings", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	...
	unique("user_settings_userId_key").on(table.userId),
]);
...
export const emailThreadsInFridayAi = fridayAi.table("email_threads", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	...
	unique("email_threads_gmailThreadId_key").on(table.gmailThreadId),
]);

export const customerEmailsInFridayAi = fridayAi.table("customer_emails", {
	id: serial().primaryKey().notNull(),
	...
	index("idx_customer_emails_customer_id").using("btree", table.customerId.asc().nullsLast().op("int4_ops")),
	index("idx_customer_emails_thread_id").using("btree", table.gmailThreadId.asc().nullsLast().op("text_ops")),
]);

export const customerConversationsInFridayAi = fridayAi.table("customer_conversations", {
	id: serial().primaryKey().notNull(),
	customerId: integer().notNull(),
	conversationId: integer().notNull(),
	createdAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	uniqueIndex("idx_customer_conversations_customer").using("btree", table.customerId.asc().nullsLast().op("int4_ops")),
]);

export const customerProfilesInFridayAi = fridayAi.table("customer_profiles", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	leadId: integer(),
	...
	index("idx_customer_profiles_email").using("btree", table.email.asc().nullsLast().op("text_ops")),
	index("idx_customer_profiles_lead_id").using("btree", table.leadId.asc().nullsLast().op("int4_ops")),
	index("idx_customer_profiles_user_id").using("btree", table.userId.asc().nullsLast().op("int4_ops")),
]);

export const emailPipelineStateInFridayAi = fridayAi.table("email_pipeline_state", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	...
	unique("email_pipeline_state_threadId_userId_key").on(table.threadId, table.userId),
	index("idx_pipeline_state_stage_user").using("btree", table.stage.asc().nullsLast(), table.userId.asc().nullsLast()),
]);

export const emailPipelineTransitionsInFridayAi = fridayAi.table("email_pipeline_transitions", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	threadId: varchar({ length: 255 }).notNull(),
	...
	index("idx_pipeline_transitions_thread_user").using("btree", table.threadId.asc().nullsLast().op("text_ops"), table.userId.asc().nullsLast().op("int4_ops")),
]);

export const analyticsEventsInFridayAi = fridayAi.table("analytics_events", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	eventType: varchar({ length: 100 }).notNull(),
	eventData: jsonb(),
	...
	index("idx_analytics_user_id").using("btree", table.userId.asc().nullsLast().op("int4_ops")),
	index("idx_analytics_event_type").using("btree", table.eventType.asc().nullsLast().op("text_ops")),
]);

export const userPreferencesInFridayAi = fridayAi.table("user_preferences", {
	id: serial().primaryKey().notNull(),
	userId: integer().notNull(),
	...
	unique("user_preferences_userId_key").on(table.userId),
]);
...
// Type aliases
export type EmailPipelineState = typeof emailPipelineStateInFridayAi.$inferSelect;
export type InsertEmailPipelineState = typeof emailPipelineStateInFridayAi.$inferInsert;
export type EmailPipelineTransition = typeof emailPipelineTransitionsInFridayAi.$inferSelect;
export type InsertEmailPipelineTransition = typeof emailPipelineTransitionsInFridayAi.$inferInsert;
export type AnalyticsEvent = typeof analyticsEventsInFridayAi.$inferSelect;
export type InsertAnalyticsEvent = typeof analyticsEventsInFridayAi.$inferInsert;
export type UserPreferences = typeof userPreferencesInFridayAi.$inferSelect;
export type InsertUserPreferences = typeof userPreferencesInFridayAi.$inferInsert;
