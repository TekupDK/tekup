...
export async function routeAI(options: AIRouterOptions & { userId?: number }): Promise<AIResponse> {
  ...
  if (context) {
    ...
    if (context.selectedThreads && context.selectedThreads.length > 0) {
      contextParts.push(`User has ${context.selectedThreads.length} email thread(s) selected`);
      if (context.selectedThreads.length <= 5) {
        contextParts.push(`Selected thread IDs: ${context.selectedThreads.join(", ")}`);
      }
    }
    ...
    if (context.previewThreadId) {
      contextParts.push(`Preview modal open for thread: ${context.previewThreadId}`);
    }

    if (context.selectedLabels && context.selectedLabels.length > 0) {
      contextParts.push(`Filtered by labels: ${context.selectedLabels.join(", ")}`);
    }
  ...
  const messagesWithSystem = hasSystemPrompt
    ? messages
    : [{ role: "system" as const, content: getFridaySystemPrompt() + contextString }, ...messages];
