 ...
 */
function buildKnowledgeContext(results: KnowledgeResult[]): string {
  if (!results || results.length === 0) {
    return '';
  }

  const contextParts = results.map((result: KnowledgeResult, index: number) => {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const knowledgeResults: KnowledgeResult[] = await searchTekupVault(message);
    ...
    if (knowledgeResults.length > 0) {
      const citations = knowledgeResults.map((result: KnowledgeResult, index: number) => {
        const doc = result.document;
        return `[${index + 1}] \`${doc.repository}/${doc.path}\``;
      }).join('\n');
    ...
    return NextResponse.json({
      ...
      sources: knowledgeResults.map((r: KnowledgeResult) => ({
