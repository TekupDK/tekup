...
/**
 * Enrich user prompt with TekupVault context
 */
export async function enrichPromptWithContext(query: string, limit: number = 3) {
  const searchResults = await searchTekupVault(query, limit);
  const context = formatSearchResults(searchResults);
  const sources = extractCitations(searchResults);
