...
export default function Home() {
  ...
  useEffect(() => {
    const loadChatHistory = () => {
      try {
        const savedMessages = localStorage.getItem('tekup-chat-history');
        if (savedMessages) {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      try {
        localStorage.setItem('tekup-chat-history', JSON.stringify(messages));
      } catch (error) {
  ...
  const clearChatHistory = () => {
    setMessages([]);
    try {
      localStorage.removeItem('tekup-chat-history');
    } catch (error) {
